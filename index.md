...
Object.entries(lakeFiles).forEach(([name, path]) => {
  ...
  fetch(path)
    ...
    .catch(() => console.warn("Skipping:", name));
...
function resetHighlight() {
  ...
  allMarkers.forEach(m => {
    const color = COLORS[m.projectCategory];
    m.setStyle({
      radius: 8,
      color: "#000",
      fillColor: color
  ...
  Object.entries(allPolygons).forEach(([name, p]) => {
    p.setStyle({
      color: "#333",
      weight: 1.5,
      fillColor: "#999",
...
// =========================
const projects = [/* KEEP YOUR ORIGINAL FULL LIST HERE */];
...
projects.forEach(project => {
  ...
  project.locations.forEach(loc => {
    ...
    // 🚫 SKIP marker if polygon exists
    if (allPolygons[loc.name]) return;
